...
func escapeAppleScriptString(_ input: String) -> String {
    // Escape characters that have special meaning in AppleScript strings
    return input.replacingOccurrences(of: "\\", with: "\\\\") // Must be first to avoid double-escaping
               .replacingOccurrences(of: "\"", with: "\\\"")
               .replacingOccurrences(of: "'", with: "\\'")
...
class SocketCommunication {
    ...
    private func handleConnection(clientSocket: Int32, configManager: ConfigurationManager?) {
        ...
        do {
            ...
            case .addAppleScript:
                logInfo("Received command to add AppleScript")
                
                if let name = commandMessage.arguments?["name"] {
                    // Check if AppleScript with this name already exists
                    var scriptsAS = configMgr.config.scriptsAS
                    if let index = scriptsAS.firstIndex(where: { $0.name == name }) {
                        // Update existing AppleScript
                        scriptsAS[index] = AppConfiguration.ScriptAppleScript(name: name, action: "")
                        configMgr.config.scriptsAS = scriptsAS
                        configMgr.saveConfig()
                        
                        let response = "AppleScript '\(name)' updated"
                        write(clientSocket, response, response.utf8.count)
                    } else {
                        // Add new AppleScript
                        scriptsAS.append(AppConfiguration.ScriptAppleScript(name: name, action: ""))
                        configMgr.config.scriptsAS = scriptsAS
                        configMgr.saveConfig()
                        
                        let response = "AppleScript '\(name)' added"
                        write(clientSocket, response, response.utf8.count)
                    }
                    ...
                    let response = "Missing name for AppleScript"
                    write(clientSocket, response, response.utf8.count)
                }
            }
...
struct AppConfiguration: Codable {
    ...
    struct ScriptAppleScript: Codable {
        ...
        // Default initializer for new AppleScripts
        init(name: String, action: String, moveTo: String? = "", noEsc: Bool? = nil, actionDelay: Double? = nil, triggerVoice: String? = "", triggerApps: String? = "", triggerModes: String? = "", triggerLogic: String? = "or") {
            self.name = name
            self.action = action
            self.moveTo = moveTo
...
class RecordingsFolderWatcher: @unchecked Sendable {
    ...
    private func readAndProcessMetaJson(at path: String) {
        ...
        if let result = json["result"],
           ...
           !String(describing: result).isEmpty {
            ...
            for ascript in configManager.config.scriptsAS {
                logInfo("[TriggerEval] Checking AppleScript action: name=\(ascript.name), triggerVoice=\(ascript.triggerVoice ?? "nil"), triggerApps=\(ascript.triggerApps ?? "nil"), triggerModes=\(ascript.triggerModes ?? "nil"), triggerLogic=\(ascript.triggerLogic ?? "nil")")
                let (matched, strippedResult) = triggersMatch(for: ascript, result: String(describing: result), modeName: modeName, frontAppName: frontAppName, frontAppBundleId: frontAppBundleId)
                if matched {
                    matchedTriggerActions.append((action: ascript, name: ascript.name, strippedResult: strippedResult))
                }
            }
            ...
            if !matchedTriggerActions.isEmpty {
                ...
                if let insert = action as? AppConfiguration.Insert {
                    ...
                    logInfo("[TriggerEval] Executing AppleScript action: \(ascript.name)")
                    self.processAppleScriptAction(ascript, metaJson: updatedJson)
                    self.handleMoveToSetting(folderPath: (path as NSString).deletingLastPathComponent, activeInsert: nil)
                }
    ...
    private func processAppleScriptAction(_ asAction: AppConfiguration.ScriptAppleScript, metaJson: [String: Any]) {
        let (processedAction, _) = self.processAction(asAction.action, metaJson: metaJson)
        
        // Create a task to execute the AppleScript
        let task = Process()
        task.launchPath = "/usr/bin/osascript"
        ...
        do {
            try task.run()
            // Don't wait for completion - run asynchronously
            logInfo("AppleScript '\(asAction.name)' launched asynchronously")
        } catch {
            logError("Failed to execute AppleScript action: \(error)")
        }
        
        // Handle ESC key press if not disabled
        if !(asAction.noEsc ?? false) {
            self.simulateEscKeyPress(activeInsert: nil)
        }
        
        // Handle action delay
        if let delay = asAction.actionDelay, delay > 0 {
            Thread.sleep(forTimeInterval: delay)
        }
...
func triggersMatch<T>(for action: T, result: String, modeName: String?, frontAppName: String?, frontAppBundleId: String?) -> (matched: Bool, strippedResult: String?) {
    ...
    let (triggerVoice, triggerModes, triggerApps, triggerLogic, actionName) = {
        ...
        case let ascript as AppConfiguration.ScriptAppleScript:
            return (ascript.triggerVoice, ascript.triggerModes, ascript.triggerApps, ascript.triggerLogic, ascript.name)
        default:
            return ("", "", "", "or", "unknown")
        }
    }()
