...
class ClipboardMonitor {
    ...
    private struct EarlyMonitoringSession {
        ...
        let ignoreClipboardUntil: Date  // Ignore clipboard changes until this time (1s blackout period)
    }
    ...
    func startEarlyMonitoring(for recordingPath: String) {
        ...
        let ignoreUntil = sessionStartTime.addingTimeInterval(1) // Ignore clipboard for 1 second
    ...
    private func monitorClipboardChangesForSession(recordingPath: String) {
        ...
        if currentChangeCount != sessionData.lastSeenChangeCount {
            ...
            if !shouldIgnore && !isInIgnoreWindow {
                ...
                logDebug("[ClipboardMonitor] Ignoring clipboard change during 1s blackout window (\(String(format: "%.2f", timeRemaining))s remaining)")
